...
# Arquitetura por Containers Isolados, Protegidos e Inquebráveis
# Capa de Cadastro + Pesquisa Instantânea para Visitantes + Perfil Social Geral
# Marketplace Híbrido + Carteira Multi-Moedas + Clube de Cupons PPE + LGPD
# ==============================================================================
...
# ==============================================================================
# ENGENHARIA DE ISOLAMENTO: EXECUTOR SEGURO DE BLOCOS POR CONTAINER
# ==============================================================================
def executar_bloco_seguro(
    nome_bloco: str, funcao_bloco, *args, container=None, **kwargs
):
  """Executa um módulo em um container isolado e protegido.

  Se o módulo falhar, a aplicação continua funcionando perfeitamente sem derrubar a tela.
  """
...
def obter_segredo_critico(chave: str):
  """Garante a interrupção segura caso uma credencial obrigatória esteja ausente."""
...
# ==============================================================================
# 2. MOTOR DE CARTEIRA MULTI-MOEDAS E SANITIZAÇÃO DE DADOS
# ==============================================================================
class GeralJaEngine:
...
# ==============================================================================
# 3. CONFIGURAÇÃO DE AMBIENTE & CONEXÃO FIRESTORE MASTER
# ==============================================================================
st.set_page_config(
    page_title="GeralJá | Ecossistema Integrado & Perfil Social Geral",
    page_icon="🇧🇷",
    layout="wide",
...
# ==============================================================================
# 4. AUTENTICAÇÃO GOOGLE & PROCESSAMENTO DE COMPARTILHAMENTO
# ==============================================================================
...
# ==============================================================================
# 5. CONSTANTES, REGRAS & FUNÇÕES AUXILIARES DE INTELIGÊNCIA
# ==============================================================================
LAT_REF = -23.5505
LON_REF = -46.6333
...
# ==============================================================================
# 6. CONFIGURAÇÃO VISUAL, TEMAS E TOPO ULTRACOMPACTO
# ==============================================================================
...
st.markdown(
    ...
    ' font-weight:700;">ECOSSISTEMA INTEGRADO DE SERVIÇOS E PERFIL SOCIAL'
    " GERAL</small></div>",
...
# ==============================================================================
# 7. FUNÇÕES MODULARES DE CONTEÚDO (CONTAINERS INDEPENDENTES & ISOLADOS)
# ==============================================================================


# --- MÓDULO 0: BUSCA AUTOMÁTICA & CAPA DE CADASTRO PARA VISITANTES ---
def modulo_busca_e_gps():
  # Capa de Cadastro / Banner de Entrada
  is_logged = st.session_state.get("auth", False)
  user_id_ativo = st.session_state.get("user_id", "")

  if not is_logged:
    st.markdown(
        """
        <div class="hero-banner">
            <h2 style="margin:0 0 8px 0; color:#FFFFFF; font-size:22px;">✨ Bem-vindo ao GeralJá!</h2>
            <p style="margin:0 0 12px 0; font-size:14px; opacity:0.95;">
                Pesquise serviços e ofertas à vontade como visitante! 
                Crie seu <b>Perfil Social Geral</b> para ganhar <b>GeralCoins 🪙</b>, resgatar cupons e interagir com o bairro.
            </p>
        </div>
        """,
        unsafe_allow_html=True,
  ...
  st.markdown("### 🏙️ Pesquise o que você precisa no Grajaú agora")
  ...
  termo_busca = c1.text_input(
      "O que você procura? Ex: 'Mecânico', 'Pizza', 'Eletricista' ou 'Cano"
      " estourado'",
      key="main_search_v6",
      placeholder="Digite aqui para buscar automaticamente...",
  )
  ...
  # A pesquisa acontece automaticamente ao digitar
  if termo_busca:
    with st.status(
    ...
    if not lista_ranking:
      ...
      for p in lista_ranking:
        ...
        link_zap = criar_link_zap(
            zap_limpo,
            f"Olá {p.get('nome')}, vi seu perfil social no GeralJá e gostaria de"
        ...
        selo_texto = " | 🏆 Perfil Elite" if is_elite else ""
        st.markdown(
            f"""
...
# --- MÓDULO 1: CLUBE DE CUPONS & VITRINE SOCIAL PPE (VISUALIZAÇÃO ABERTA / INTERAÇÃO RESTRITA AO PERFIL) ---
def modulo_clube_cupons():
  st.markdown("### 🎟️ Clube de Cupons & Vitrine Social PPE")
  st.caption(
      "Você pode ver todas as ofertas livremente! Para curtir, comentar,"
      " divulgar e resgatar cupons acumulando moedas, informe seu Perfil"
      " Social Geral abaixo."
  )
  ...
  # Identificação do Perfil Social do Morador/Visitante
  id_morador = st.text_input(
      "Seu WhatsApp ou Perfil Social (Para consultar saldo e acumular moedas):",
  ...
  for of in ofertas_exemplo:
    ...
    with st.container():
      ...
      with col_o2:
        ...
        # Botões Sociais PPE (Visitante visualiza, mas precisa de Perfil para interagir)
        c1, c2, c3 = st.columns(3)
        if c1.button("👍 Curtir (+1 GC)", key=f"btn_lk_{post_id}"):
          if not id_morador:
            st.warning(
                "⚠️ Cadastre seu Perfil Social ou informe seu WhatsApp no topo"
                " para interagir e ganhar moedas!"
            )
          else:
        ...
        with c2:
          with st.popover("💬 Comentar (+2 GC)"):
            ...
            if st.button("Enviar", key=f"sub_comm_{post_id}"):
              if not id_morador:
                st.warning("⚠️ Informe seu Perfil Social/WhatsApp acima!")
        ...
        with c3:
          with st.popover("📢 Divulgar (+3 GC)"):
            ...
            if st.button("Confirmar Compartilhamento", key=f"sub_sh_{post_id}"):
              if not id_morador:
                st.warning("⚠️ Cadastre seu Perfil Social para pontuar!")
              else:
                zap_m = limpar_whatsapp(id_morador)
                ok, msg = processar_ppe(
                    zap_m,
        ...
        # Resgate do Cupom (Restrito ao Perfil Cadastrado)
        if st.button(
            f"🛒 Resgatar Cupom de Desconto ({of['item']})", key=f"btn_resg_{post_id}"
        ):
          if not id_morador:
            st.warning(
                "⚠️ Informe seu WhatsApp / Perfil Social no topo para resgatar"
                " este cupom!"
            )
          else:
            zap_m = limpar_whatsapp(id_morador)
...
# --- MÓDULO 2: CADASTRO DE PERFIL SOCIAL GERAL (MORADOR, PROFISSIONAL OU COMÉRCIO) ---
def modulo_cadastro_parceiro():
  st.markdown("### 🚀 Cadastro de Perfil Social Geral")
  st.caption(
      "Crie seu perfil para aparecer no mapa do bairro, ganhar GeralCoins e"
      " interagir com as ofertas locais."
  )
  ...
  with st.form("form_profissional", clear_on_submit=False):
    ...
    nome_input = col1.text_input(
        "Nome Completo / Nome do Negócio", value=nome_inicial
    )
    zap_input = col2.text_input("WhatsApp (DDD + Número sem espaços)")

    email_input = st.text_input("E-mail para Acesso", value=email_inicial)

    col3, col4 = st.columns(2)
    cat_input = col3.selectbox("Especialidade / Ramo Principal", cats_dinamicas)
    senha_input = col4.text_input("Senha de Acesso", type="password")

    desc_input = st.text_area(
        "Apresentação do seu Perfil / Bio / Horário de Atendimento"
    )
    tipo_input = st.radio(
        "Tipo de Perfil Social Geral",
        [
            "👤 Morador / Consumidor",
    ...
    btn_acao = st.form_submit_button(
        "✅ Salvar Perfil Social e Ganhar Bônus", use_container_width=True
    )
  ...
  if btn_acao:
    ...
    if not nome_input or not zap_limpo or not senha_input:
      ...
      try:
        ...
        st.balloons()
        st.success(
            "🎉 Perfil Social criado com sucesso! Bônus de 20 GeralCoins"
            " creditado."
        )
      except Exception as e:
...
# --- MÓDULO 3: PAINEL DO PERFIL SOCIAL & GESTOR DE PEDIDOS ---
def modulo_painel_parceiro():
  if not st.session_state.get("auth"):
    st.subheader("🚀 Acesso ao seu Perfil Social")
    col1, col2 = st.columns(2)
    l_zap = col1.text_input("WhatsApp Cadastrado", key="login_zap_v6")
    l_pw = col2.text_input("Senha", type="password", key="login_pw_v6")

    if st.button(
        "Entrar no Meu Perfil", key="btn_login_v6", use_container_width=True
    ):
    ...
    st.write(
        f"### Olá, {d.get('nome', 'Membro')}! <small"
        f" style='font-size:14px; color:#64748B;'>({d.get('tipo', 'Perfil Social')})</small>",
        unsafe_allow_html=True,
    )
    ...
    with st.expander("📝 Editar meu Perfil Social & Galeria"):
      with st.form("perfil_v6"):
        cats_atuais_edit = carregar_categorias_dinamicas()
        n_nome = st.text_input("Nome Comercial / Pessoal", d.get("nome", ""))
        n_area = st.selectbox(
...
# --- MÓDULO 4: TORRE DE CONTROLE ADMIN ---
def modulo_admin_torre_controle():
  if not st.session_state.get("admin_logado"):
    ...
    try:
      ...
      if qtd_pendentes > 0:
        st.error(
            f"🚨 **Atenção:** {qtd_pendentes} perfis aguardando aprovação!"
        )
        msg_central = (
      ...
      if not df.empty:
        ...
        c_a1.metric("Total de Membros", len(df))
        c_a2.metric(
            "Cliques Totais",
...
# --- MÓDULO 6: FINANCEIRO (OPCIONAL/SECRETO) ---
...
# Execução Isolada nos Containers de Cada Aba
with menu_abas[0]:
...
with menu_abas[2]:
  executar_bloco_seguro("Cadastro de Perfil Social", modulo_cadastro_parceiro)

with menu_abas[3]:
  executar_bloco_seguro("Painel do Perfil Social", modulo_painel_parceiro)
...
<div class="footer-container">
    ...
    <p>© 2026 GeralJá | Ecossistema Blindado LGPD & Perfil Social Geral</p>
</div>
""",
    unsafe_allow_html=True,
)
...
with st.expander("📄 Transparência e Privacidade (LGPD)"):
  ...
  st.markdown("""
    **Como tratamos seus dados:**
    1. **Finalidade:** Seus dados são usados exclusivamente para conectar você a clientes e moradores no Grajaú.
